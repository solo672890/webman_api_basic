...
namespace app\extends\jwt;



use app\extends\jwt\src\BeforeValidException;
use app\extends\jwt\src\ExpiredException;
use app\extends\jwt\src\JWT;
use app\extends\jwt\src\Key;
use app\extends\jwt\src\SignatureInvalidException;
...
class JwtToken
{
     ...
     * @param string $val
     * @return mixed|string
     * @throws \app\extends\jwt\JwtTokenException
     */
    ...
    public static function refreshToken(&$_extend = []): array
    {
        ...
        try {
            ...
            throw new JwtTokenException('刷新令牌无效',401021);
        } catch (BeforeValidException $beforeValidException) {
            throw new JwtTokenException('刷新令牌尚未生效',401022);
        } catch (ExpiredException $expiredException) {
            throw new JwtTokenException('刷新令牌会话已过期，请再次登录！',401023);
        } catch (UnexpectedValueException $unexpectedValueException) {
            throw new JwtTokenException('刷新令牌获取的扩展字段不存在',401024);
        }
    ...
    public static function verify(int $tokenType = self::ACCESS_TOKEN, ?string $token = null): array
    {
        ...
        try {
            ...
            throw new JwtTokenException('身份验证会话已过期，请重新登录！',30002);
        } catch (UnexpectedValueException $unexpectedValueException) {
            throw new JwtTokenException('获取的扩展字段不存在',401014);
        } catch (JwtTokenException | \Exception $exception) {
    ...
    private static function getTokenFromHeaders(): string
    {
        ...
        if (!$authorization || 'undefined' == $authorization) {
            ...
            if (!isset($config['is_support_get_token']) || false === $config['is_support_get_token']) {
                throw new JwtTokenException('请求未携带authorization信息1',401000);
            }
            $authorization = request()->get($config['is_support_get_token_key']);
            if (empty($authorization)) {
                throw new JwtTokenException('请求未携带authorization信息2',401000);
            }
        ...
        if (2 != count(explode(' ', $authorization))) {
            throw new JwtTokenException('Bearer验证中的凭证格式有误',401000);//中间必须有个空格
        }
    ...
    private static function _getConfig(): array
    {
        ...
        if (empty($config)) {
            throw new JwtTokenException('jwt配置文件不存在');
        }
        return $config;
    }
